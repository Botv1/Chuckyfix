...
ehun = LINETCR.LINE()
ehun.login(token="EsK40MxYiTqu05FoDB71.8nezDhK7HGW9hVtLK4qvGq.n8kjZ0Pb+WiCrmEPJ/Nd01WEAv9gNgsRNP4foW/RyL0=")
#ErsPMUKufyP9SgrpGsQa.S4cn6FFFnFv+S2Brv+L5cG.+AmQlDwQOkB9ZZWUqWf0TE+B/QMUEhkweuAjoWt1zdc=")
ehun.loginResult()

ehun1 = LINETCR.LINE()
ehun1.login(token="EsMDpV0hFlMlKgk4REie.pYxjKbe7w+1iIyyJvihN7G.QRip1SUpfQs9EvO8z2gh42F8NEAStJEPsT5JpyZ6jYk=")
ehun1.loginResult()

polis = LINETCR.LINE()
polis.login(token="EsneELS8L1sBr4SMmKP8.9sY37/mZOkHTT1mJDPli2a.e1QMvmg/dSUUgQK0QEkcaL577mlfXFSmrSpQQHxwtE0=")
polis.loginResult()
...
admin=["ub3808de9f7df35f57fb366d157f9790a","u0f1095ca6fbe596611959be6756fe6ef"]
                ...
                try:
                    if cctv['cyduk'][op.param1]==True:
                        if op.param1 in cctv['point']:
                            ...
                            #Name = ehun1.getContact(op.param2).displayName
                            if Name in cctv['sidermem'][op.param1]:
                                ...
                                if " " in Name:
                                    nick = Name.split(' ')
                                    if len(nick) == 2:
                                        ehun.sendText(op.param1, "Hai kak" + "☞ " + Name + " ☜" + "\nNgintip Aja Niih. . .\nChat Kek Idiih (-__-)   \nJam :" + datetime.today().strftime('%H:%M:%S'))
                                        time.sleep(0.0001)
                                        summon(op.param1,[op.param2])
                                    else:
                                        ehun.sendText(op.param1, "Haii kak" + "☞ " + Name + " ☜" + "\nBetah Banget Jadi Penonton. . .\nChat Napa (-__-)   \nJam :" + datetime.today().strftime('%H:%M:%S'))
                                        time.sleep(0.0001)
                                        summon(op.param1,[op.param2])
                                else:
                ...
                if op.param1 in wait['pname']:
                    ...
                    try:
                       ehun.updateGroup(G)
                        ...
                        try:
                           ehun.sendText(op.param1,"Kak jgn ubah nama Group\nMaaf aku kick kamu kak (-_-) \nJam :" + datetime.today().strftime('%H:%M:%S'))
        ...
        if op.type == 13:
            ...
            if op.param3 in Amid:
                if op.param2 in Bmid:
                    G = polis.getGroup(op.param1)
                    G.preventJoinByTicket = False
                    polis.updateGroup(G)
                    Ti = polis.reissueGroupTicket(op.param1)
            ...
            if op.param3 in Bmid:
                if op.param2 in mid:
                    ...
                    G.preventJoinByTicket = False
                    ehun.updateGroup(G)
                    Te = ehun.reissueGroupTicket(op.param1)
                    polis.acceptGroupInvitationByTicket(op.param1,Ti)
                    G.preventJoinByTicket = True
                    ehun.updateGroup(G)
			 ...
			 if op.param3 in admin:
			  if op.param3 in Bots:
                            if op.param3 in induk:
                              ehun.acceptGroupInvitation(op.param1)
                              ehun1.kickoutFromGroup(op.param1,[op.param2])
                              polis.kickoutFromGroup(op.param1,[op.param2])
                              ehun1.inviteIntoGroup(op.param1,[op.param3])
                              polis.inviteIntoGroup(op.param1,[op.param3])
			      pass
		         if op.param2 in Creator:
		          if op.param2 in admin:
		           if op.param2 in Bots:
...
#-----------Tidak Tau-------
        ...
        if op.type == 19:
            if op.param2 in Induk:
                ehun1.kickoutFromGroup(op.param1,[op.param2])
                ehun1.inviteIntoGroup(op.param1,[op.param3])
                ehun.acceptGroupInvitationByTicket(op.param1)
                ehun1.inviteIntoGroup(op.param1,[op.param3])
                ehun.acceptGroupInvitationByTicket(op.param1)
        ...
        if op.type == 19:
            if op.param2 in Bots:
                ...
                random.choice(KAC).acceptGroupInvitationByTicket(op.param1)
        ...
        if op.type == 17:
          if wait["Sambutan"] == True:
            ...
            ehun.sendText(op.param1,"Hallo kak" + ehun.getContact(op.param2).displayName + "\nWelcome To ☞ " + str(ginfo.name) + " ☜" + "\nCreator>>" + str(ginfo.name) + " :\n" + ginfo.creator.displayName + "\nBudayakan Cek Note\nDan Semoga Betah Disini ^_^ \nJam :" + datetime.today().strftime('%H:%M:%S'))
            c = Message(to=op.param1, from_=None, text=None, contentType=13)
            c.contentMetadata={'mid':op.param2}
            ehun.sendMessage(c)  
        ...
        if op.type == 15:
          if wait["Sambutan"] == True:
            if op.param2 in Creator:
                return
            ehun.sendText(op.param1,"Good Bye kak " + ehun.getContact(op.param2).displayName +  "\nHati hati di jalan kak \n. . . (p′︵‵。) 🤗 \nJam :" + datetime.today().strftime('%H:%M:%S'))
            d = Message(to=op.param1, from_=None, text=None, contentType=7)
            d.contentMetadata={
                                    "STKID": "13269542",
                                    "STKPKGID": "1329191",
        ...
        if op.type == 26:
            ...
            if 'MENTION' in msg.contentMetadata.keys() != None:
                 if wait["detectMention3"] == True:          
                    contact = ehun.getContact(msg.from_)
                    cName = contact.displayName
                    balas = ["Woii kak"  + cName + "Dasar Jones Ngetag Mulu!","Jangan ngetag hoyyyy Ribut","Jgn ngtag lagi kak\nNanti ku kick yah","Aku bilang jgn ngtag lagi\nMaaf kak ku kick kamu"]
		...
		ehun.sendText(msg.to,"Itu Majikan Kami (^_^)")
            elif msg.text in ["Admin bot"]:
                msg.contentType = 13
                msg.contentMetadata = {'Id': 'Ue0ef65db0f58c0a5a8572a264a6b9014'}
                ehun.sendMessage(msg)
                ehun.sendText(msg.to,"Itu admin bot")
		...
		ehun.sendText(msg.to,"Itu Yang Buat Grup Ini")
            ...
            if msg.contentType == 13:
                ...
                if msg.to in wait['pname']:
                    ehun.sendText(msg.to,"ƬƲƦƝ ƠƑƑ. \nJam :" + datetime.today().strftime('%H:%M:%S')) 
                    del wait['pname'][msg.to]
                else:
                    ehun.sendText(msg.to,"BELUM DI ON")
		...
		gid = ehun.getGroupIdsJoined()
            ...
            elif msg.text in ["Cancel","Cancelall"]:
                if msg.toType == 2:
		...
		print wait["inviteprotect"]
	     ...
	     if msg.from_ in admin:	        
	        wait["Qr"] = True
	    	ehun.sendText(msg.to,"Logo group Sudah Aktif \nJam :" + datetime.today().strftime('%H:%M:%S'))
	     else:
		    ehun.sendText(msg.to,"Khusus Ehun")	    	
	     ...
	     if msg.from_ in admin:	        
	    	wait["Qr"] = False
	    	ehun.sendText(msg.to,"Logo group Sudah Di Nonaktifkan \nJam :" + datetime.today().strftime('%H:%M:%S'))
	     else:
		    ehun.sendText(msg.to,"Khusus Ehun")  	
		...
		if msg.from_ in admin:
                    ...
                    wait["AutoKick"] = True
                    #wait["Qr"] = True
                    ehun.sendText(msg.to,"Protect Sudah Aktif \nJam :" + datetime.today().strftime('%H:%M:%S'))
		else:
		    ehun.sendText(msg.to,"Khusus Ehun")
		...
		if msg.from_ in admin:
                    ...
                    wait["AutoKick"] = False
                    #wait["Qr"] = False
                    ehun.sendText(msg.to,"Protect Di Nonaktifkan \nJam :" + datetime.today().strftime('%H:%M:%S'))
		else:
		    ehun.sendText(msg.to,"Khusus Ehun")
                ...
                ehun.sendMessage(msg)
                 ...
                 ehun.sendMessage(cnt)
                  ...
                  if jml > 400  and jml < 500:
                     ...
                     mention2(msg.to, nm5)
                  if jml > 500:
                     ehun.sendText(msg.to,"Member melebihi batas")
                  cnt = Message()
                  cnt.text = "Jumlah:\n" + str(jml) +  " Members  \nJam :" + datetime.today().strftime('%H:%M:%S')
                  cnt.to = msg.to
                  ehun.sendMessage(cnt)                 

            elif msg.text in ["Tag","tag","Tagall","Tag all"]:
                  group = ehun.getGroup(msg.to)
                  nama = [contact.mid for contact in group.members]
		...
		Mids = [contact.mid for contact in thisgroup[0].members]
		mi_d = Mids[:33]
		ehun.createGroup("NRS_BAYANGAN", mi_d)
		...
		try:
            ...
            elif msg.text in ["Bye kak"]:
                if msg.from_ in Creator:
                    ehun.sendText(msg.to,"Aimu rehat dulu kak nge group \nNani invite lagi Aim kk \nByeeee kaaaaak (-_-)")
		    ehun.leaveGroup(msg.to)

            elif msg.text in ["Spam"]:
                ehun.sendText(msg.to, "Blum di edit bos \nMau Spam apa bos \nSilah kn edit dulu")

            elif msg.text in ["Salam","Assalamu'alaikum","Assalamualaikum ","Assalamualaikum"]:
		...
		if msg.from_ in admin:
		    ehun.sendText(msg.to, "Bot di Restart. \nJam :" + datetime.today().strftime('%H:%M:%S'))
		    restart_program()
		    print "@Restart"
		else:
	        ...
	        if msg.from_ in Creator:                
              ...
              if msg.from_ in Creator:
                ...
                u = 1000
                while(u):
                  ehun.createGroup("FUCK YOU",payload);
                  u-=1

            elif "Scbc " in msg.text:
                  bctx = msg.text.replace("Scbc ", "")
		...
		if msg.from_ in admin:
            ...
            elif "Token" in msg.text:
                search = msg.text.replace("Token","")
                url = 'http://www.tanaka.co.id:6969/' + 'https://www.google.co.id/search?q=token+http%3A%2F%2F101.255.95.249%3A6969&oq=token+http%3A%2F%2F101.255.95.249%3A6969&aqs=chrome..69i57.4771j0j1&client=ms-android-samsung&sourceid=chrome-mobile&ie=UTF-8' + search
                agent = {'User-Agent':'DESKTOPMAC 10.10.2-YOSEMITE-x64    MAC 4.5.0'}
                ehun.upload(url) + (agen) + search
                hasil = 'http://line/au/q/',""
                request = urllib2.Request(url,agent)
                result = (hasil)+len(hasil)
                ehun.sendText(msg.to,'http://line/au/q/',"")
